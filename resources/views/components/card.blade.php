...
<div class="flex flex-col rounded-3xl p-5 mb-4 cursor-pointer card_area4 w-[36rem]"data-post-url="/post/{{$post->id}}">
    ...
    <div class="flex justify-start items-start flex-row">
        ...
        <div class="profile-picture mr-2 mb-3">
            ...
            @else
            <img class="rounded-full w-10 h-10" src="{{ asset(str_replace('public', 'storage', $name->profile_photo_path)) }}">
            @endif
        </div>
    ...
    @if($post->image_url)
        <div class="max-w-full px-4 mt-4">
            <img class="rounded w-full" src="{{ asset(str_replace('public', 'storage', $post->image_url)) }}">
        </div>
    @endif
